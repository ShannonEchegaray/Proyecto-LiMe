...
const Item = ({ item }) => {
  const { isInSavedCart, addSavedCart, removeSavedCart } =
  ...
  return (
    <div className="itemContainer flex-start">
      <div
        className="corazon"
        onClick={() =>
          isInSavedCart(item.id) ? removeSavedCart(item.id) : addSavedCart(item)
        }
      >
        <FavoriteIcon
          sx={
            isInSavedCart(item.id) ? { color: "red" } : { color: "whitesmoke" }
          }
        />
      </div>
