      ...
      <div>
        ...
        <form action="" className="formularioBuscador">
          ...
          <button className="botonBuscador">
            <div className="iconoBuscador">
              <BiSearch size={"16px"} />
            </div>
          </button>
        </form>
        <h3 style={{ fontSize: "0.95em" }}>
          Enviós gratis en 24 hs a partir de $4000
        </h3>
            ...
            <li className="categorias">
              ...
              <ul className="bg-black rounded overflow-hidden m-0 p-0">
                {categories.map((el, index) => (
                  <li
                    key={index}
                    className="bg-gray-800 px-2 py-1.5 hover:bg-gray-500"
                  >
                    <Link className="text-white" to={`/${el.categoria}`}>
                      {el.categoria}
                    </Link>
                  </li>
                ))}
